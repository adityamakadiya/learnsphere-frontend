...
function BrowseCourses() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        try {
          ...
          coursesData = coursesResponse.data.data;
          console.log("Courses:", coursesData); // Debug
          setCourses(coursesData);
        } catch (courseErr) {
          console.error(
            "Failed to fetch courses:",
        ...
        try {
          ...
          enrolledIds = enrollmentsResponse.data.data.map((e) => e.course.id);
          console.log("Enrollments:", enrollmentsResponse.data.data); // Debug
          setEnrolledCourseIds(enrolledIds);
        } catch (enrollErr) {
          console.error(
            "Failed to fetch enrollments:",
  ...
  const handleCategorySelect = (categoryId) => {
    ...
    const fetchCourses = async () => {
      ...
      try {
        const coursesUrl = categoryId
          ? `/students/courses?categoryId=${categoryId}`
          : "/students/courses";
        const coursesResponse = await api.get(coursesUrl);
        const coursesData = coursesResponse.data.data;
        console.log("Courses for category:", coursesData); // Debug
        setCourses(coursesData);
      } catch (courseErr) {
        console.error(
          "Failed to fetch courses:",
  ...
  const handleEnroll = async (courseId) => {
    try {
      ...
      console.log("Enroll response: Success"); // Debug
      setEnrolledCourseIds([...enrolledCourseIds, courseId]);
      alert("Enrolled successfully!");
    } catch (err) {
